...
biclique <- function(filename, lleast = 1, rleast = 1, version = 1, getclique = 1, filetype = 0)  #may change this function name to bic.profile
{
    ...
    profile = paste(" biclique \t Number \n")
   	...
   	for (i in seq(1, nelems-9, 3)){
   	    profile = paste(profile, "K", profile.raw[i+1], ",", profile.raw[i+2], "\t", profile.raw[i+3], "\n", sep="")
   	}
    ...
    profile = paste(profile, "Maximum edge biclique       : ", "K", profile.raw[nelems-3], ",", profile.raw[nelems-2], "\n", sep="")
    profile = paste(profile, "Maximum vertex biclique     : ", "K", profile.raw[nelems-1], ",", profile.raw[nelems], "\n", sep="")
